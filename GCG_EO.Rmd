...
## Background
Discrete classification assign to dominant class.
Pixels often mixed signatures of several land covers.
Workarounds: SMA, regression.
Practical aspects...

## Exercise

provide mixed spectrum A (Landsat). 

spectrum <- data.frame('wavelength'=c(515, 600, 680, 885, 1660, 2300), 
                      'reflectance'=c())
ggplot(profile, aes(x=wavelength, y=values, group=DOY)) + 
      geom_line(aes(color=DOY), lwd=1) + 
      scale_y_continuous(name="DN") + 
      scale_x_continuous(name="Wavelength (nm)") + 
      theme_bw()
      

endmembers A, B, C? soil, water, grass
endmembers <- data.frame('wavelength'=c(515, 600, 680, 885, 1660, 2300), 
                        'soil'=c(),
                        'grass'=c(),
                        'shade'=c())
...
ggplot(profile, aes(x=wavelength, y=values, group=DOY)) + 
      ...
      scale_y_continuous(name="DN") + 
      scale_x_continuous(name="Wavelength (nm)") + 
      scale_colour_discrete(name="Acquisition DOY") +
      theme_bw()

synthetic mixing -> apply unmixing

```{r sma_1, echo=TRUE, eval=FALSE}

fraction_a <- 0.34
fraction_b <- 0.41
fraction_c <- 0.25
fraction_a + fraction_b + fraction_c == 1
target <- fraction_a * A + fraction_b * B + fraction_c * C

```
find out the error term?

select endmembers for: 
deciduous, conifer, understory, shade?
