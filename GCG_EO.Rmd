...
- Parameterize a compositing function and create your own composite from multiple Landsat scenes
...
One of the first global composites using Landsat data was produced using the WELD compositing approach (Roy et al. 2011, https://doi.org/10.1016/j.rse.2009.08.011). 
...
## Parametric pixel-based compositing

- a method to produce a gap-free image of a (large) area
- representing a specific point in time (target date)
- best pixel selected according to multiple parameters (e.g. how close to target date, distance to clouds)
weighted scoring allows for flexible parametrization according to user´s needs and study area characteristics
...
We use linear functions to determine the suitability of an observation.  
...
To minimize the likelihood of pixels being affected by clouds, we favor observations that aren't in close proximity to clouds. The cloud distance scoring function below assigns a suitability score of 1 as soon as the distance to the next cloud is 100 or more pixels. An observation that has a distance of 60 pixels (=1800m) to the next cloud receives a cloud distance suitability score of 0.6. 
...
Using the values from the exemplary plots above yields:  
$$score = 0.4 * 0.5 + 1 * 0.2 + 0.6 * 0.3$$
$$score = 0.58$$

![Resulting suitability scores for each image](fig/s04_suitability_score_weighted.png)

6) Use the best observation (= highest score) for the final composite.
...
<details>
  <summary>Click here to see the code for the main script calling the parametric compositing function</summary>
...
Relying on your insights from collecting training data on forest types, define two target days of the year (DOY) to capture different phenological stages. These will be used as target_date parameters later on.  
Make a decision concerning the compositing parameters explained in 1c, 1d and 1e.  
...
In this exercise, we will deal with image classification using the Random Forests algorithm. Specifically, we will use your training data and your pixel-based composites from the last exercises to map forest types in the Western Beskids. We will assess the performance of multiple classification models through the out of bag error and will investigate variable importances.  

#### 1) Loading and preparing training data; training the classification model
...
c. Train a randomForest() classification model with the data.frame created in the prior step.  

### 2) Training additional models based on different input data 
Repeat 1) to train an additional randomForest models based on

a. …the other composite.  

b. …a stack of both composites.  

### 3) Investigating the models and classification
The model object resulting from 2) contains a wealth of information on the model parameters and performance. Assess the out of bag (OOB) error estimates of the trained models (1b, 2a, 2b) by inspecting the err.rate attribute of your model objects.
a. Which model has the lowest OOB error?
b. How does the OOB behave when increasing the number of trees in your model (ntrees)? You can access the OOB per number of trees included via err.rate. Use this information to determine a suitable value for ntrees.
c. Which of the four classes has the highest OOB errors?

### 4) Training a final model and investigating variable importances
a. Train a final model with the best combination of images (3a) and ntrees (3b).
...
### 5) Classification
Perform a classification of the image stack using the predict() function. Write the resulting map to disk in ENVI format. When doing so, consider choosing the appropriate datatype argument.
...
### 1) Participate in the quiz in moodle. In case you had wrong answers, please discuss with your classmates.

### 2) Creating a stratified set of reference samples
a. Load your forest type map in R. Create a set of reference samples in R using sampleStratified(). We want to have a stratified equalized sample of 20 pixels per class. Specify the size parameter accordingly.  
Specify sp = TRUE to receive a spatial points object and na.rm = TRUE to ignore unclassified regions. Write the points to disk using writeOGR().

### 3) Determine class labels for the reference samples
a. Load the point shapefile in QGIS and add two fields of type Integer: classID and confID.
b. Link QGIS and GoogleEarth using GearthView. Visit each point and determine the class label according to the high resolution imagery. Enter the class and confidence labels in the attribute table. Work efficiently through the points and save your changes regularly.
...
### 4) Assessing class proportions and creating a confusion matrix
a. In R, assess the class proportions of your map. To do so, use the freq()function to get pixel counts. Use these for calculating class propotions (0-1). Remember to exclude NAs, so the proportions of your 4 classes sum up to 1.
b. Load the points in R. Create a confusion matrix from the resulting data.frame using table().

### 5) Deriving accuracies and evaluating results
Copy & paste the values from your confusion matrix as well as the class proportions into the Excel spreadsheet.   
...
We provided the following datasets at O:/…/S07/data/…  
...
a. Visualize the Landsat images in QGIS. Try out a true color (RGB: 3-2-1), as well as a 4-5-3 false color combination.
b. Optional: Derive additional features from your data. Do you want to classify based on the spectral bands? What about vegetation indices, principal components, tasseled cap components?

### 3) Training data collection

a. Consider that the way you collect your training data varies with the approach you choose in 1).
b. Collecting training data is time consuming - try to be efficient. Make use of the QGIS Info-Tool to compare spectral profiles of the three images. You can also link QGIS and Google Earth through the GearthView plugin. For exact pixel locations, use the Landsat pixel grid we provided.
...
### 6) Reporting results  
We collect your main results in this table: https://ethercalc.org/572rg3qh93t2 Please fill in all required fields, i.e. your name and selected approach (e.g. post-classification comparison), overall accuracy and class accuracies in decimal values (e.g. 0.75), as well as class areas in hectares.
