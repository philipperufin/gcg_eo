...
#############################################################################
# Helper function for Gaussian DOY scoring
gauss_score <- function(param, target, max_offset){
...
The `e1071` package offers automated parameter optimization for several classification algorithms. Use the `tune.randomForest()` function to find the best combination of `mtry`and `ntree` for your classification problem. The decision is made on an accuracy score derived via 10-fold cross-validation.
<!--
...
rf.tune <- tune.randomForest(classID~., data = train.df, ntree=seq(100, 500, 50), mtry=c(2:10))
...
svm.tune <- tune.svm(classID~., data = train.df, kernel = 'radial', gamma = 10^(-3:3), cost = 10^(-3:3))
