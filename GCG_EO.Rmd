...
![Post-deforestation land uses in the study region include intensive soy cultivation (left) and rather extensive cattle ranching (right). Images: Philippe Rufin](fig/s08_lu.png)

From a remote sensing perspective, separating cropland and grassland is difficult due to similar spectral signatures, coupled with diverse management regimes.
Looking at single points in time, croplands and pastures are difficult to discriminate. By creating an intra-annual time series, we might be able to capture exactly these differences in land surface phenology.

![Intra-annual time series of Tasseled Cap Greenness for 2010.](fig/s08_tcg_ts.gif)


Those differences can be emphasized by aggregation into spectral-temporal metrics. Spectral-temporal metrics are descriptive statistics of the spectral behaviour over time. They provide an efficient means to capture seasonal variations of land surface dynamics, while reducing the impact of clouds and cloud-shadows. 

Can be calculated from spectral bands, but also indices or TC components
Different metrics might be more or less useful depending on the application.

![Exemplary metrics calculated from intra-annual time series of Tasseled Cap Greenness.](fig/s08_stm.png)
...
b. …spectemp/: eight spectral temporal metrics for TCB, TCG and TCW describing
...
```extract()``` the values at your training locations from the temporal stacks (e.g., …/ts_stacks/TCB.tif). Compare the phenological behavior of the classes with boxplots or line plots. Use the following code to import the acquisition dates of the Landsat images:
...
Summarize the main characteristics of the three classes over the growing season and identify major differences. Please add your insights in comments in your script.
Select spectral-temporal metrics that capture the differences between croplands, pastures, and others well. Select at most six different metrics from the ones provided.
...
How to do that: A number of summarizing functions can be applied directly to raster stacks (```mean()```, ```max()```, ```min()```, ```range()```). If you want to calculate metrics other than those, you need to convert your raster stack into a matrix using as.matrix() and perform your calculations on this matrix using ```apply()```. You can then write the results into a new raster, which you create using ```raster()``` and assign the results of your computation with the *vals* parameter.  
See the following code template for a detailed example to compute the median Tasseled Cap Greenness. Computation might take several minutes - make use of the processing time by preparing the subsequent tasks.
...
a. Train a Random Forest model based on your selected / computed spectral-temporal metrics. As always, make an informed decision about the ntree and mtry parameters.

b. Produce a map using your Random Forest model and ```mask()``` the result with last week´s forest map, so only non-forest areas in 2010 are left. Write the result to disk.

### 5) Accuracy Assessment and explore land change processes
...
b. Combine the resulting map with your forest change map from last week to get a better insight into the land change processes of the study area. Calculate the fractions of deforested land occupied by pasture or cropland for each of the two time periods. Was forest primarily replaced by pastures or by cropland? How did this process change over the periods 2001-2005 and 2006-2010? Please enter your main results in [this table](https://ethercalc.org/sputdinazkck).
