...
parametric_compositing <- function(img_list, target_date, 
  ...
  ### DOY score
  # ...
  score <- data.frame('DOY' = c(1:365),
                      'DOY_offset' = abs(target_DOY - c(1:365)),
                      'DOY_score' = dnorm(c(1:365), mean=target_DOY, sd=max_DOY_offset/2))
  
  # Remove DOYs outside valid range
  score$DOY_score[score$DOY_offset > max_DOY_offset] <- NA
  
  # Standardize scores
  score$DOY_score <- (score$DOY_score - min(score$DOY_score, na.rm=T))/diff(range(score$DOY_score, na.rm=T), na.rm=T)
  
  # Append scores to img_list
  img_list <- merge(img_list, score, by='DOY')
  ...
  p <- ggplot(score, aes(x=DOY, y=DOY_score)) +
          geom_line() +
...
The optimal parameters are detected based on a classification accuracy, derived by predicting a withheld fraction of the training data. To minimize the sampling bias, we specify a 5-fold cross-validation, where the sampling and prediction procedure is repeated five times.
<!--
