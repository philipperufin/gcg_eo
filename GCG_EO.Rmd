...
Subject to the following constraints:0 <= fi<= 1CONSTRAINT 1f1+ f2+ ...fn= 1CONSTRAINT 2Researchers have noted that 2 is easy to implement, but 1 is not.As a result, most studies only implement the 2nd Constraint in the model and then apply the 1st constraint to the results.

There  is  no  significant  occurrence  of  multiple  scattering  between  the  different surface components.•Each  surface  component  within  the  image  has  sufficient  spectralcontrast  to allow their separation.•In each pixel the total land cover is unity.•Each surface component (endmember) is known

Unter der Bedingung, dass jedes Photon nur mit einer Fläche interagiert, kann angenommen werden, dass der Anteil am Signal einzig vom Anteil der Fläche am Pixel bestimmt wird.
...
Neben den einzelnen Residuen bzw. dem Residualspektrum dient der kann RMSE als Maß für die Güte der Mischungsanalyse: 
...
# Define endmember sprectra from composite images at DOY 074 and 196
...
ggplot(endmembers) + 
      ...
      #geom_line(aes(x=wavelength, y=beech_summer), color='forestgreen', lwd=1) + 
      geom_line(aes(x=wavelength, y=spruce_074), color='steelblue4', lwd=1) + 
      #geom_line(aes(x=wavelength, y=spruce_summer), color='steelblue3', lwd=1) +
...
```{r matrix_inversion, eval=F, echo=T}
library(MASS)

EM <- as.matrix(endmembers[,c('beech_074','spruce_074','shade')])
MS <- as.matrix(endmembers[,'model_spectrum'])

EM_fractions <- ginv(EM) %*% MS
residuals <- EM %*% fractions - endmembers$model_spectrum
...
# Session 08: Multi-temporal change detection
...
# Session 09: Spectral-temporal metrics
