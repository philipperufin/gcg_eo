...
Parametric best-pixel-compositing allows to use a set of rules (parameters) to determine the observations best suited for analysis. These parameters can include information about distance to clouds, temporal proximity to a target date or seasonal consistency. We use best-pixel-compositing to produce gap-free images of a (large) area, 
...
Compositing can be extremely helpful for creating gap-free images in regions that are scarce in data density.
This method allows to create cloud free, radiometrically consistent image datasets over large areas from the Landsat archive.
...
### 1)	Parameterization
We provide a function which allows you to produce cloud-free best-pixel composites for a pre-defined target DOY. This function requires one input data.frame (see code template on how to create it) and eight input parameters: 
...
### 2) Exploring the script
Open the parametric_compositing.R script and take time to read through it in groups. Make sure you understand how the function operates. Seek the help pages of functions you don´t know, test single parts of the function and discuss questions with each other.
...
### 5) Setting parameters and running the script
...
### 6) Improving the user-friendliness of the script
...
### 7) Second run for target date 2
Repeat the compositing for the second target DOY you specified in 4) and write the results to disk.

### 8) Visual inspection and evaluation of results
